 ...
 * @retval A pointer to the part of the string after the "BLE_" prefix if it
 * exists, otherwise a pointer to the original string.
 */
...
uint8_t stm32wb_at_client_Query(stm32wb_at_BLE_CMD_t cmd)
{
    ...
    if (cmd == BLE_TEST) {
        ...
        status = stm32wb_at_ll_Transmit((uint8_t *) &client_buff_tx[0],
                                        (uint16_t) strlen(client_buff_tx));
        (void) memset(&client_buff_tx[0], 0, sizeof(client_buff_tx));
...
uint8_t stm32wb_at_client_Process_rx_frame(char *str)
{
    ...
    if (strcmp(str_local, AT_CMD_STATUS_OK) == 0) {
        ...
        if (strncmp(
                str_local, remove_prefix(AT_BLE_CMD_STRING[client_current_cmd]),
                strlen(remove_prefix(AT_BLE_CMD_STRING[client_current_cmd]))) ==
            0) {
            str_local +=
                strlen(remove_prefix(AT_BLE_CMD_STRING[client_current_cmd]));
            ...
            if (strncmp(str_local, AT_GET, strlen(AT_GET)) == 0) {
                ...
                case BLE_VER: {
                    ...
                    if (token != NULL) {
                        strncpy(param.module_type, token,
                                sizeof(param.module_type) - 1);
                    ...
                    token = strtok(NULL, AT_SEPARATOR);
                    if (token != NULL) {
                        strncpy(param.production_date, token,
                                sizeof(param.production_date) - 1);
                        param.production_date[sizeof(param.production_date) -
                                              1] = '\0';
                    } else {
                        status |= 1U;
                    }
                ...
                case BLE_PROFILE: {
                    stm32wb_at_BLE_PROFILE_t param;
                    char *token;
                    status = 0;

                    token = strtok(str_local, AT_SEPARATOR);
                    if (token)
                        param.profile = (uint16_t) strtol(token, NULL, 0);
                    else
                        status |= 1U;

                    if (!status) {
                        status = stm32wb_at_BLE_PROFILE_cb(&param);
                        client_current_cmd = BLE_NONE;
                    }
                    break;
                }
